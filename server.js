...
import {
  streamExportToResponse,
  streamImportFromRequest,
  analyzeTree,
} from './Treestream.js';
...
jsonRouter.get('/me', requireAuth, async (req, res) => {
  try {
...
jsonRouter.patch('/me', requireAuth, async (req, res) => {
  try {
...
jsonRouter.get('/trees', requireAuth, async (req, res) => {
  try {
    const snap = await db
      .collection('trees')
...
jsonRouter.post('/trees', requireAuth, async (req, res) => {
  try {
...
jsonRouter.get('/trees/:id', requireAuth, async (req, res) => {
  try {
...
jsonRouter.put('/trees/:id', requireAuth, async (req, res) => {
  try {
...
jsonRouter.delete('/trees/:id', requireAuth, async (req, res) => {
  try {
    const ref = db.collection('trees').doc(req.params.id);
    const doc = await ref.get();
    if (!doc.exists)                         return res.status(404).json({ error: 'Дерево не знайдено' });
...
app.get('/api/trees/:id/export-stream', requireAuth, async (req, res) => {
  try {
    const doc = await db.collection('trees').doc(req.params.id).get();
    if (!doc.exists)                         return res.status(404).json({ error: 'Дерево не знайдено' });
    if (doc.data().ownerId !== req.user.uid) return res.status(403).json({ error: 'Немає доступу' });
...
app.post('/api/trees/:id/import-stream', requireAuth, async (req, res) => {
  try {
    const doc = await db.collection('trees').doc(req.params.id).get();
    if (!doc.exists)                         return res.status(404).json({ error: 'Дерево не знайдено' });
    if (doc.data().ownerId !== req.user.uid) return res.status(403).json({ error: 'Немає доступу' });
...
app.get('/api/trees/:id/analyze', requireAuth, async (req, res) => {
  try {
    const doc = await db.collection('trees').doc(req.params.id).get();
    if (!doc.exists)                         return res.status(404).json({ error: 'Дерево не знайдено' });
    if (doc.data().ownerId !== req.user.uid) return res.status(403).json({ error: 'Немає доступу' });
