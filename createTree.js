...
document.getElementById('mName').addEventListener('keydown', e => { if (e.key === 'Enter') savePerson(); });
...
function openEditModal(id) {
  ...
  setVal('mName', p.name || ''); setVal('mGender', p.gender || 'm');
  setVal('mAlive', p.alive || 'alive'); setVal('mBirth', p.birth || '');
  setVal('mBirthPlace', p.birthPlace || ''); setVal('mDeath', p.death || '');
  setVal('mDeathPlace', p.deathPlace || ''); setVal('mNationality', p.nationality || '');
  setVal('mReligion', p.religion || ''); setVal('mOccupation', p.occupation || '');
  setVal('mMarital', p.marital || ''); setVal('mSocialClass', p.socialClass || '');
  setVal('mLocation', p.location || ''); setVal('mAwards', p.awards || '');
  setVal('mEduLevel', p.eduLevel || ''); setVal('mSchool', p.school || '');
  setVal('mEduFrom', p.eduFrom || ''); setVal('mEduTo', p.eduTo || '');
  setVal('mSpecialty', p.specialty || ''); setVal('mNotes', p.notes || '');
...
function closeModal() { addModal.classList.remove('open'); photoData = null; }

function resetModalFields() {
  ['mName','mBirth','mBirthPlace','mDeath','mDeathPlace','mNationality','mReligion',
   'mOccupation','mLocation','mAwards','mSchool','mEduFrom','mEduTo','mSpecialty','mNotes','mSources']
    .forEach(id => setVal(id, ''));
  setVal('mGender', 'm'); setVal('mAlive', 'alive');
  setVal('mMarital', ''); setVal('mSocialClass', ''); setVal('mEduLevel', '');
...
function savePerson() {
  ...
  const data = {
    name, gender: getVal('mGender'), alive: getVal('mAlive'),
    birth: getVal('mBirth'), birthPlace: getVal('mBirthPlace'),
    death: getVal('mDeath'), deathPlace: getVal('mDeathPlace'),
    nationality: getVal('mNationality'), religion: getVal('mReligion'),
    occupation: getVal('mOccupation'), marital: getVal('mMarital'),
    socialClass: getVal('mSocialClass'), location: getVal('mLocation'),
    awards: getVal('mAwards'), eduLevel: getVal('mEduLevel'),
    school: getVal('mSchool'), eduFrom: getVal('mEduFrom'),
    eduTo: getVal('mEduTo'), specialty: getVal('mSpecialty'),
  ...
  if (editingId !== null) {
    ...
    const id = nextId++;
    data.id = id;
    data.x  = 40 + (people.length % 4) * 170;
    data.y  = 40 + Math.floor(people.length / 4) * 130;
    people.push(data);
    closeModal(); renderAll(); selectPerson(id);
  }
  scheduleSave();
}
...
function renderAll() {
  ...
  renderNodes(); renderConnections(); renderSidebar();
}
...
function renderSidebar() {
  ...
  if (people.length === 0) {
    ...
    people.forEach(p => {
      ...
      `;
      item.querySelector('.person-del').addEventListener('click', e => { e.stopPropagation(); deletePerson(p.id); });
  ...
  if (connections.length === 0) {
    ...
    connections.forEach((c, i) => {
      const from = getById(c.from), to = getById(c.to);
      if (!from || !to) return;
      const item = document.createElement('div');
...
const MARITAL_LABELS = { single:'Неодружений/а', married:'Одружений/а', divorced:'Розлучений/а', widowed:'Вдівець/Вдова' };
const SOCIAL_LABELS  = { peasant:'Селянин/Селянка', worker:'Робітник/Робітниця', intelligentsia:'Інтелігенція', military:'Військовий/а', clergy:'Духовенство', nobility:'Дворянство', merchant:'Купець/Купчиха' };
const EDU_LABELS     = { none:'Без освіти', primary:'Початкова', secondary:'Середня', vocational:'Проф.-технічна', higher:'Вища', postgrad:'Аспірантура/Докторантура' };
...
function showProfilePanel(id) {
  ...
  `;
  const genRows = [ppRow('Стать', p.gender === 'm' ? 'Чоловік' : 'Жінка'), ppRow('Народження', [p.birth, p.birthPlace].filter(Boolean).join(', ')), ppRow('Смерть', p.death ? [p.death, p.deathPlace].filter(Boolean).join(', ') : ''), ppRow('Національність', p.nationality), ppRow('Релігія', p.religion)].join('');
  if (genRows) html += `<div class="pp-section"><div class="pp-section-title">📋 Загальні дані</div>${genRows}</div>`;
  const socRows = [ppRow('Професія', p.occupation), ppRow('Сімейний стан', MARITAL_LABELS[p.marital] || ''), ppRow('Соц. стан', SOCIAL_LABELS[p.socialClass] || ''), ppRow('Проживання', p.location), ppRow('Нагороди', p.awards)].join('');
  if (socRows) html += `<div class="pp-section"><div class="pp-section-title">👔 Соціальний статус</div>${socRows}</div>`;
  const eduPeriod = p.eduFrom || p.eduTo ? [p.eduFrom, p.eduTo].filter(Boolean).join(' – ') : '';
  const eduRows = [ppRow('Рівень', EDU_LABELS[p.eduLevel] || ''), ppRow('Заклад', p.school), ppRow('Роки навчання', eduPeriod), ppRow('Спеціальність', p.specialty)].join('');
  if (eduRows) html += `<div class="pp-section"><div class="pp-section-title">🎓 Освіта</div>${eduRows}</div>`;
  if (p.notes) html += `<div class="pp-section"><div class="pp-section-title">📝 Нотатки</div><div class="pp-notes">${p.notes}</div></div>`;
  if (p.sources) html += `<div class="pp-section">${ppRow('📚 Джерела', p.sources)}</div>`;
  html += `<div class="pp-section"><div class="pp-section-title">🔗 Зв'язки</div>${relsHtml}</div>`;
...
function zoom(delta) {
  ...
  canvas.style.transform = `scale(${scale})`; canvas.style.transformOrigin = '0 0';
  svgLines.style.transform = `scale(${scale})`; svgLines.style.transformOrigin = '0 0';
}
function resetView() { scale = 1; canvas.style.transform = ''; svgLines.style.transform = ''; }

document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { selectedId = null; renderAll(); closeModal(); closeProfile(); }
  if ((e.key === 'Delete' || e.key === 'Backspace') && selectedId
      && document.activeElement.tagName !== 'INPUT'
      && document.activeElement.tagName !== 'TEXTAREA') { deletePerson(selectedId); }
  if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); saveTree(); }
});

function loadDemoData() {
  people = [
    { id: nextId++, name: 'Василь Петренко',  birth: '1940', death: '2005', gender: 'm', alive: 'dead',  birthPlace: 'Полтава', occupation: 'Агроном',   socialClass: 'peasant',        eduLevel: 'secondary', nationality: 'Українець', notes: 'Учасник відбудови після Другої світової.', x: 80,  y: 80  },
    { id: nextId++, name: 'Марія Петренко',   birth: '1945', death: '',     gender: 'f', alive: 'alive', birthPlace: 'Вінниця', occupation: 'Вчителька', socialClass: 'intelligentsia', eduLevel: 'higher',    school: 'Вінницький педагогічний', specialty: 'Початкова освіта', x: 260, y: 80  },
    { id: nextId++, name: 'Олексій Петренко', birth: '1968', death: '',     gender: 'm', alive: 'alive', birthPlace: 'Київ',    occupation: 'Інженер',   socialClass: 'intelligentsia', eduLevel: 'higher',    school: 'КПІ', specialty: 'Електроніка', location: 'Київ', x: 80,  y: 240 },
    { id: nextId++, name: 'Наталія Коваль',   birth: '1972', death: '',     gender: 'f', alive: 'alive', birthPlace: 'Харків',  occupation: 'Лікарка',   socialClass: 'intelligentsia', eduLevel: 'higher',    school: 'Харківський медичний університет', specialty: 'Педіатрія', x: 260, y: 240 },
  ];
  connections = [
    { from: 1, to: 2, type: 'partner' }, { from: 1, to: 3, type: 'parent' },
    { from: 2, to: 3, type: 'parent'  }, { from: 3, to: 4, type: 'partner' },
  ];
}